			...
			if(!empty($q))
			{
				foreach ($q as $key=>$value)
				{
					...
					$id = $this->primary_id;
                                        ...
                                        if ($value->status == 0){
                                            $accept = base_url($this->controller.'/accept/'.$this->utility->encode($value->$id));
					...
					$nestedData['status'] = $statusText;
                                        if ($value->status == 0) {
                                            ...
                                            $nestedData['manage'] = "<a href='$statusAction' class='btn  btn-warning  btn-xs confirm-statuschange'>Block</a><a href='$delete' class='btn  btn-warning  btn-xs confirm-delete'>Delete</a>";
                                        } elseif ($value->status == 2) {
